...
public class WorkLogEntryControllerGetAllTodosIt extends IntegrationTestBase {
	...
	public void testGivenExsistingUserThenTodosIsRecieved() {
		...
		MockHttpServletRequestBuilder request = get("/users/" + TESTED_USER.getUsername() + "/worklogentrys").accept(MediaType.APPLICATION_JSON);
		// WHEN
	...
	public void testGivenNonExsistingUserThenNotFoundRecieved() {
		// GIVEN
		authenticateTestedUser();
		MockHttpServletRequestBuilder request = get("/users/nonexsistinguser/worklogentrys").accept(MediaType.APPLICATION_JSON);
		// WHEN
		ResultActions resultActions = performWithToken(request);
		// THEN
